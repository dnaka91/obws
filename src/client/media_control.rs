...
impl<'a> MediaControl<'a> {
    ...
    /// Get the length of media in milliseconds. Supports ffmpeg and vlc media sources
    /// (as of OBS v25.0.8) Note: For some reason, for the first 5 or so seconds that the media is
    /// playing, the total duration can be off by upwards of 50ms.
    ///
    /// - `source_name`: Source name.
    pub async fn get_media_duration(&self, source_name: &str) -> Result<responses::MediaDuration> {
        self.client
            .send_message(RequestType::GetMediaDuration { source_name })
            .await
    }
    ...
    ///
    /// - `source_name`: Source name.
    pub async fn get_media_time(&self, source_name: &str) -> Result<responses::MediaTime> {
        self.client
            .send_message(RequestType::GetMediaTime { source_name })
            .await
    }
    ...
    /// (as of OBS v25.0.8) Note: Due to processing/network delays, this request is not perfect.
    ...
    /// Get the current playing state of a media source. Supports ffmpeg and vlc media sources
    /// (as of OBS v25.0.8)
    ///
    /// - `source_name`: Source name.
    pub async fn get_media_state(&self, source_name: &str) -> Result<responses::MediaState> {
        self.client
            .send_message(RequestType::GetMediaState { source_name })
            .await
    }
}
